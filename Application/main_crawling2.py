...
def main():
    ...
    list_query_endpoint = ["query/class.sparql", "query/property.sparql", "query/label_class.sparql",
                           "query/label_property.sparql", "query/property_domain_range.sparql", "query/subclass.sparql", "query/subproperty.sparql",
    ...
    for ep in list_endpoint:
        ...
        if ep == list_endpoint[-1]:
                ...
                for graph in list_graph:                        
                    ...
                    if res != {} and nb != 0:
                        for r in res["results"]["bindings"]:
                            classe = (r['class']['value'])
                            
                            # ajout du repertoire de la classe
                            classe_dir = os.path.join(class_dir, return_file_uri(classe))
                            os.makedirs(classe_dir, exist_ok=True)

                            # ajout du repertoire labels de la classe
                            labels_dir = os.path.join(classe_dir, "labels")
                            os.makedirs(labels_dir, exist_ok=True)

                            if res2 != {} and nb2 != 0:
                                for r2 in res2["results"]["bindings"]:
                                    if (r2["class"]["value"]) == classe:
                                        label = (r2["label"]["value"])
                                        lang = (r2["lang"]["value"])

                                        lang_dir = os.path.join(labels_dir, lang)

                                        if not os.path.exists(lang_dir):
                                            # ajout du repertoire de la langue
                                            os.makedirs(lang_dir)
                                        # Creation du fichier du label
                                        f = open(lang_dir + '/' + return_file_uri(label), "w")
                                        f.close()

                            # ajout du fichier des propriétés ou la classe est domain et celui ou la classe est range
                            if res4 != {} and nb4 != 0:
                                for r4 in res4["results"]["bindings"]:
                                    property = r4["property"]["value"]
                                    # domain
                                    if (r4["domain"]["value"] == classe):
                    ...
                    # Ajout du repertoire des proprietes
                    property_dir = os.path.join(graph_dir,"properties")
                    os.makedirs(property_dir,exist_ok=True)
                    # Lancement de la requete pour recuperer les proprietes
                    nb, q, res = launch_query(setting_query(list_query_endpoint[1], ep, graph, 100, False), 1)
                    print(len(res3['results']['bindings']))
                    if res != {} and nb != 0:
                        for r in res["results"]["bindings"]:
                            propriete = (r['property']['value'])
                            
                            # On cree le repertoire de la propriete
                            propriete_dir = os.path.join(property_dir, return_file_uri(propriete))
                            os.makedirs(propriete_dir, exist_ok=True)


                            # ajout du repertoire labels de la propriete
                            labels_dir = os.path.join(propriete_dir, "labels")
                            os.makedirs(labels_dir, exist_ok=True)

                            if res3 != {} and nb3 != 0:
                                for r3 in res3["results"]["bindings"]:
                                    if (r3["property"]["value"]) == propriete:
                                        label = (r3["label"]["value"])
                                        lang = (r3["lang"]["value"])
                            ...
                            # ajout du fichier des domains et ranges
                            if res4 != {} and nb4 !=0:
                                for r4 in res4["results"]["bindings"]:
                                    if r4["property"]["value"] == propriete:
                                        domain = r4["domain"]["value"]
                                        rang = r4["range"] ["value"]
