...
def main():
    ...
    list_query_endpoint = ["query/class.sparql", "query/property.sparql", "query/label_class_lang.sparql",
    ...
    for ep in list_endpoint:
        ...
        if ep == list_endpoint[8]:
                ...
                compared_query = {}
                nb, q, res = launch_query(setting_query(list_query_graph[1], ep, None, 100, True), None)
                if res != {}:
                    list_graph = res

                # liste des different graphes
                for r in res["results"]["bindings"]:
                    graph = r["graph"]["value"]
                    print (graph)
                    ...
                    nb3, q3, res3 = launch_query(setting_query(list_query_endpoint[4], ep, graph, 100, False), 1)
                    ...
                    if res != {} and nb != 0:
                        for r in res["results"]["bindings"]:
                            ...
                            if res2 != {} and nb2 != 0:
                                print ("labels de classe non nul")
                                for r2 in res2["results"]["bindings"]:
                                    print (r2["class"]["value"])
                                    print (r2["label"]["value"])
                                    print (r2["lang"]["value"])
                                    print ()
                                    if (r["class"]["value"]) == classe:
                                        label = (r["label"]["value"])
                                        lang = (r["lang"]["value"])
